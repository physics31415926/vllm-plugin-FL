...
# Skip if vllm_metax already registered this
if "GatedDeltaNetAttention" not in op_registry_oot:
    ...
    # ---------------------------------------------------------------------------------
    # Totally the same as GatedDeltaNetAttention, but this version is to make torch
    # compile happy on forward. `has_attr()` in forward has some issue in TorchDynamo
    # when generate guard branch.
    #
