...
#
# Patch vllm's fused_moe utils._int8_quantize to use ops.scaled_int8_quant
# for per-token quantization on MACA, which avoids the per_token_quant_int8
# function that is not available on MetaX.
# TODO: Remove when vllm upstream aligns int8 quant path across platforms.
...
def _int8_quantize(
    ...
    if block_shape is None:
        if per_act_token:
            # ┌-------------------------- Metax Modification -------------------------┐
            # Use ops.scaled_int8_quant instead of per_token_quant_int8 on MACA
