...
#
# Patch GroupedTopKRouter._compute_routing to use MACA-optimised grouped topk kernels.
# C extension ops (mx_ops.sgl_fused_moe_gate_opt, mx_ops.grouped_topk) are provided
# by the vllm_metax package; pure Python fallback (maca_grouped_topk) lives here.
# TODO: Remove rocm_aiter dependency guard once vllm upstream cleans up the import.
...
# C extensions from vllm_metax (requires vllm_metax installed with .so files)
    ...
    if scoring_func == "sigmoid":
        ...
        if (
            ...
            mx_ops.sgl_fused_moe_gate_opt(
                gating_output, bias_bf16, topk_values, topk_indices,
                topk, renormalize, num_expert_group, topk_group, 0, routed_scaling_factor,
            )
        else:
        ...
        topk_values, topk_indices = mx_ops.grouped_topk(
            ...
            routed_scaling_factor,
            e_score_correction_bias,
            0,  # scoring_func=0, scores already computed
        )
    else:
