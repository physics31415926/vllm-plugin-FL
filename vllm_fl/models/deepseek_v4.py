...
# Adapted for vllm-plugin-FL: removes hard SM10/deep_gemm/mxfp4 dependencies
# so DeepSeek-V4 can run on A800 and other non-Blackwell hardware.
import typing
...
# P-hardware: Mxfp4MoEMethod is only available on SM10 (Blackwell).
# Lazy-import it so the module can be loaded on A800 / non-Blackwell hardware.
# The actual class is only used inside DeepseekV4FP8Config.get_quant_method
# which guards with is_mxfp4_quant() before calling it.
def _get_mxfp4_moe_method():
    ...
    def get_quant_method(self, layer, prefix):
        if isinstance(layer, FusedMoE):
            ...
            if self.expert_dtype == "fp4":
                # P-hardware: lazy import — Mxfp4MoEMethod requires SM10.
                # On non-Blackwell hardware this path is unreachable because
                # fp4 checkpoints cannot be loaded without SM10 anyway.
                Mxfp4MoEMethod = _get_mxfp4_moe_method()
...
class DeepseekV4MoE(nn.Module):
    def __init__(
        ...
        self.prefix = prefix
        # P-hardware: deep_gemm MegaMoE requires SM10 (Blackwell) + deep_gemm.
        # Force-disable on non-SM10 hardware so the standard FusedMoE path is used.
        _requested_mega_moe = (
            vllm_config.kernel_config.moe_backend == "deep_gemm_mega_moe"
        )
        if _requested_mega_moe:
            cap = current_platform.get_device_capability()
            _sm10_available = cap is not None and cap[0] >= 10
            if not _sm10_available:
                import warnings
                warnings.warn(
                    "DeepSeek-V4 MegaMoE (deep_gemm) requires SM10+ (Blackwell). "
                    "Falling back to standard FusedMoE path on this hardware.",
                    stacklevel=2,
                )
                _requested_mega_moe = False
        self.use_mega_moe = _requested_mega_moe
...
class DeepseekV4Model(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        quant_config = vllm_config.quant_config
        self.config = config
        # P-hardware: mirror the same SM10 guard used in DeepseekV4MoE
        _requested_mega_moe = (
            vllm_config.kernel_config.moe_backend == "deep_gemm_mega_moe"
        )
        if _requested_mega_moe:
            cap = current_platform.get_device_capability()
            if cap is None or cap[0] < 10:
                _requested_mega_moe = False
        self.use_mega_moe = _requested_mega_moe
        if self.use_mega_moe and not vllm_config.parallel_config.enable_expert_parallel:
            raise NotImplementedError(
