...
Adapted from upstream minicpmv4_6_main.py for vllm-plugin-FL.
Only the relative imports are rewritten to absolute vllm.model_executor.models.*
paths; all logic is unchanged.
"""
...
# Absolute imports replacing the upstream relative .* imports
...
def _patch_image_processor_compat(image_processor) -> None:
    """Patch a transformers 5.7+ MiniCPMV4_6ImageProcessor with the legacy
    attribute / method interface that vLLM 0.20.2's minicpmv.py expects.

    vLLM 0.20.2 accesses these names on image_processor:
        mean, std                   → image_mean, image_std
        im_id_start, im_id_end      → '<image_id>', '</image_id>' string tokens
        im_start_token, im_end_token→ '<image>', '</image>' string tokens
        image_feature_size          → (scale_resolution // patch_size) ** 2
        get_slice_image_placeholder → builds the placeholder string
    """
    if getattr(image_processor, "_vllm_compat_patched", False):
        return  # already patched

    # 1. mean / std aliases
    for old, new in (("mean", "image_mean"), ("std", "image_std")):
        if not hasattr(image_processor, old) and hasattr(image_processor, new):
            setattr(image_processor, old, getattr(image_processor, new))

    # 2. Special token strings
    if not hasattr(image_processor, "im_id_start"):
        image_processor.im_id_start = "<image_id>"
    if not hasattr(image_processor, "im_id_end"):
        image_processor.im_id_end = "</image_id>"
    if not hasattr(image_processor, "im_start_token"):
        image_processor.im_start_token = "<image>"
    if not hasattr(image_processor, "im_end_token"):
        image_processor.im_end_token = "</image>"

    # 3. image_feature_size: tokens per patch = (scale_resolution // patch_size)^2
    if not hasattr(image_processor, "image_feature_size"):
        scale_res = getattr(image_processor, "scale_resolution", 448)
        patch_sz = getattr(image_processor, "patch_size", 14)
        image_processor.image_feature_size = (scale_res // patch_sz) ** 2

    image_processor._vllm_compat_patched = True
...
class MiniCPMV4_6ProcessingInfo(MiniCPMVProcessingInfo):
    ...
    def get_hf_processor(self, **kwargs: object):
        ...
        _patch_image_processor_compat(image_processor)
        return hf_processor
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                loaded_params.add(mapped_name)  # report merged name, not source name
                break
            else:
