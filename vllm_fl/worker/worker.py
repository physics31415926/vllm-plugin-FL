...
class WorkerFL(WorkerBase):
    def __init__(
        ...
        # FL: Log environment variables
        logger.debug("=== ENVIRONMENT VARIABLES ===")
        for k, v in sorted(os.environ.items()):
            logger.debug("%s=%r", k, v)

        # FL: Register out-of-tree ops
        register_oot_ops()

        # FL: FlagGems initialization
    ...
    def sleep(self, level: int = 1) -> None:
        # TODO(lms): rewrite CuMemAllocator for FL
        # CuMemAllocator not ported to FL yet, no-op stub
        pass
    ...
    def _maybe_get_memory_pool_context(self, tag: str) -> AbstractContextManager:
        # TODO(lms): rewrite CuMemAllocator for FL
        # CuMemAllocator not ported to FL yet, return nullcontext
        return nullcontext()
    ...
    def init_device(self):
        if self.device_config.device_type == "cuda":
            ...
            if (
                ...
                assert self.local_rank < current_platform.torch_device_fn.device_count(), (
